...
namespace Microsoft.CodeAnalysis.Formatting
{
    internal abstract class AbstractSyntaxFormattingService : ISyntaxFormattingService
    {
        ...
        protected abstract IFormattingResult CreateAggregatedFormattingResult(SyntaxNode node, IList<AbstractFormattingResult> results, IntervalTree<TextSpan, TextSpanIntervalIntrospector> formattingSpans = null);
            ...
            SyntaxNode node, OptionSet options, IEnumerable<AbstractFormattingRule> rules, IList<TextSpan> spansToFormat, CancellationToken cancellationToken)
        {
            ...
            return CreateAggregatedFormattingResult(node, new List<AbstractFormattingResult>(1) { result }, IntervalTree.Create(new TextSpanIntervalIntrospector(), spanToFormat));
        }
